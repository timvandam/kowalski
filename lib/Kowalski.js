...
class Kowalski extends Readable {
   ...
   * @param {any} data - data to provide the Information instance
   * @returns {Boolean} whether data was succesfully read
   */
  triggerInfoCollect (name, data) {
    ...
    const info = new Information(data)
    if (!(info instanceof Kowalski.Information)) return false
    if (this.reading) this.reading = this.push(info) // if reading stream the info
    else this.infoBuffer.push(info) // else save it in the buffer
    return true
  }
